...
class I18n(object):
    ...
    _all = {
        ...
        'pt_BR': {
            ...
            'strategy.auto_select': 'Estratégia inválida "{strategy}". Automaticamente selecionando "spiral"',
            ...
            '--fingerprint': 'Sua fingerprint. Veja README.md (em ingles) para instruções de como configurar.',
            ...
            '--colors_not_overwrite': 'Cores escolhidas não serão sobrepostas no Canvas.',
            '--draw_strategy': 'Estratégia de desenho. Escolha da lista de estratégias [linear, p_linear, qf, randomize, status, sketch, radiate, spiral] default: randomize',
            '--mode_defensive': 'Ativa o modo demoníaco (roda em segundo plano). Isso irá durar para sempre.',
            '--proxy_url': 'url do Proxy e porta. Ex: url:porta . DEPRECATED',
            '--proxy_auth': 'Autenticação do Proxy. Ex: usuário:senha . DEPRECATED',
            ...
            '--QR_text': 'link ou texto para gerar QR code',
            '--QR_scale': 'Proporção do QR code',
            ...
            '--point_x': 'Ponto de partida para coordenadas X em estratégias que partem de um único ponto, como radial e spiral. (Por padrão: do centro)',
            '--point_y': 'Ponto de partida para coordenadas Y em estratégias que partem de um único ponto, como radial e spiral. (Por padrão: do centro)',
            '--prioritized': 'Prioriza pixels baseado no nível de transparência do template, pixels com maior transparência são pintados primeiro pelo bot; permite definir pontos estratégicos no template',
    ...
    _locale = locale.getdefaultlocale(['LANG'])[0]
    try:
        _all[_locale]
    except KeyError:
        _locale = 'en_GB'
